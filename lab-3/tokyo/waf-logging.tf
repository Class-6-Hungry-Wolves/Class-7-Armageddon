...
resource "aws_cloudwatch_log_group" "armageddon_waf_log_group01" {
  ...
  # NOTE: AWS requires WAF log destination names start with aws-waf-logs- (must not rename this).
...
resource "aws_wafv2_web_acl_logging_configuration" "armageddon_waf_logging01" {
  ...
  redacted_fields {
    single_header { name = "set-cookie" }
  }
  redacted_fields {
    single_header { name = "x-api-key" }
  }

  depends_on = [aws_wafv2_web_acl.armageddon_cf_waf01, aws_cloudwatch_log_group.armageddon_waf_log_group01]
}
...
resource "aws_s3_bucket" "armageddon_waf_logs_bucket01" {
  ...
  bucket = "aws-waf-logs-${var.project_name}-${data.aws_caller_identity.my_self01.account_id}"
...
resource "aws_s3_bucket" "armageddon_firehose_waf_dest_bucket01" {
  ...
  bucket = "${var.project_name}-waf-firehose-dest-${data.aws_caller_identity.my_self01.account_id}"
