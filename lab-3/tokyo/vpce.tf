...
resource "aws_vpc_endpoint" "shinjuku_vpce_s3_gw01" {
  ...
  route_table_ids = [
    aws_route_table.shinjuku_private_rtb.id
  ]
...
# Explanation: CloudWatch Logs is the ship’s black box—Tokyo wants crash data, always.
...
resource "aws_vpc_endpoint" "shinjuku_vpce_monitoring01" {
  vpc_id              = aws_vpc.shinjuku-vpc.id
  ...
  tags = {
    Name = "${local.project_name}-vpce-monitoring01"
  }
}
