...
resource "aws_secretsmanager_secret" "armageddon-db-secret" {
  name                           = "${local.name_prefix}/rds/mysql" # delete the '01' or added if the previous secret is still not deleted
...
resource "aws_ssm_parameter" "armageddon-db-name-param" {
  name  = "/lab/db/name"
  type  = "String"
  value = var.db_name
...
# Will have to use interpolation in BASH script to point to log group
# Explanation: When the Falcon is on fire, logs tell you *which* wire sparked—ship them centrally.
resource "aws_cloudwatch_log_group" "armageddon-log-group" {
  name              = "/aws/ec2/${local.name_prefix}-rds-app"
  retention_in_days = 1 # Number of days you want to retain log events in the specified log group
...
# This filter counts the number of log events containing the term "ERROR" and 
# publishes them to the custom metric named "DBConnectionErrors" in the "Lab/RDSApp" namespace. 
resource "aws_cloudwatch_log_metric_filter" "error-metric-filter" {
  name           = "error-count-filter"
  log_group_name = aws_cloudwatch_log_group.armageddon-log-group.name
  pattern        = "ERROR"
...
# Explanation: Metrics are lab_1a’s growls—when they spike, something is wrong.
# NOTE: Students must emit the metric from app/agent; this just declares the alarm.
resource "aws_cloudwatch_metric_alarm" "armageddon-db-alarm" {
  alarm_name          = "${local.name_prefix}-db-connection-failure"
  alarm_description   = "Triggers when DBConnectionsErrors is greater than or equal to 3 in 5 minutes"
  ...
  statistic           = "Sum"
  threshold           = 3
  treat_missing_data  = "notBreaching" # Tells CW Alarms to treat missing data points as "healthy" or within the threshold - Stays in 'OK' state instead of 'ALARM' when no data is received

  alarm_actions = [aws_sns_topic.armageddon-sns-topic.arn]

  tags = {
    Name = "${local.name_prefix}-alarm-db-connection-failure"
  }
}
...
resource "aws_sns_topic" "armageddon-sns-topic" {
...
resource "aws_sns_topic_subscription" "armageddon-sns-sub" {
  topic_arn = aws_sns_topic.armageddon-sns-topic.arn
  protocol  = "email"
