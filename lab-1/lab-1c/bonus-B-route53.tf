...
#   armageddon_zone_id = var.manage_route53_in_terraform ? aws_route53_zone.armageddon-zone[0].zone_id : var.route53_hosted_zone_id
...
# resource "aws_route53_zone" "armageddon-zone" {
...
# resource "aws_route53_record" "armageddon-acm-validation-records" {
#   for_each = var.certificate_validation_method == "DNS" ? {
#     for dvo in aws_acm_certificate.armageddon-acm-cert.domain_validation_options :
#     dvo.domain_name => {
...
# resource "aws_acm_certificate_validation" "armageddon-acm-validation-dns-bonus" {
#   count = var.certificate_validation_method == "DNS" ? 1 : 0

#   certificate_arn = aws_acm_certificate.armageddon-acm-cert.arn

#   validation_record_fqdns = [
#     for r in aws_route53_record.armageddon-acm-validation-records : r.fqdn
#   ]
# }