...
resource "aws_vpc" "lab-1-vpc" {
  ...
  tags = {
    name = "${local.project_name_prefix}-${local.environment}-lab-1-vpc"
  }
}

# Public Subnets where our ALB will be housed

resource "aws_subnet" "lab-1-public-subnet" {
  ...
  tags = {
    Name = "${local.project_name_prefix}-${local.environment}-lab1-public-subnet"
  }
}

# Private Subnets where our EC2 lab app will reside
resource "aws_subnet" "lab-1-private-subnet" {
  vpc_id            = aws_vpc.lab-1-vpc.id
  for_each          = var.private_subnet_config
  cidr_block        = each.value.cidr_block
  availability_zone = each.value.az
  tags = {
    Name = "${local.project_name_prefix}-${local.environment}-lab1-private-subnet"
  }
}


# Private Subnets where our RDS database will reside
resource "aws_subnet" "lab-1-database-subnet" {
  vpc_id                  = aws_vpc.lab-1-vpc.id
  ...
  tags = {
    Name = "${local.project_name_prefix}-${local.environment}-lab1-database-subnet"
  }
}
...
resource "aws_internet_gateway" "lab_1_igw" {
  vpc_id = aws_vpc.lab-1-vpc.id
  tags = {
    Name = "${local.project_name_prefix}-${local.environment}-lab_1_igw"
  }
}


# Elastic IP to attach to nat gateway 
resource "aws_eip" "lab_1_nat_eip" {
  domain     = "vpc"
  depends_on = [aws_internet_gateway.lab_1_igw]
  tags = {
    Name = "${local.project_name_prefix}-${local.environment}-lab_1_nat_eip"
  }
}

# NAT Gateway to provide outbound internet access to private resources
resource "aws_nat_gateway" "lab_1_nat_gateway" {
  depends_on    = [aws_subnet.lab-1-public-subnet]
  allocation_id = aws_eip.lab_1_nat_eip.id
  subnet_id     = aws_subnet.lab-1-public-subnet["lab_1_public_subnet_1"].id
  tags = {
    Name = "${local.project_name_prefix}-${local.environment}-lab_1_nat_gateway"
  }
}

# Public Route table with corresponding default route of 0.0.0.0/0 to provide access from all IPS
resource "aws_route_table" "lab_1_public_rtb" {
  vpc_id = aws_vpc.lab-1-vpc.id
  route {
  ...
  tags = {
    Name = "${local.project_name_prefix}-${local.environment}-lab_1_public_rtb"
  }
}

# Private Route table with corresponding default route of 0.0.0.0/0 to provide access from all IPS
resource "aws_route_table" "lab_1_private_rtb" {
  vpc_id = aws_vpc.lab-1-vpc.id
  route {
    cidr_block     = "0.0.0.0/0"
    nat_gateway_id = aws_nat_gateway.lab_1_nat_gateway.id
  }
  tags = {
    Name = "${local.project_name_prefix}-${local.environment}-lab_1_private_rtb"
  }
}
...
resource "aws_route_table_association" "lab_1_public_rtb_association" {
  for_each       = aws_subnet.lab-1-public-subnet
  subnet_id      = each.value.id
  route_table_id = aws_route_table.lab_1_public_rtb.id
}

# Private Route Table association for private subnets
resource "aws_route_table_association" "lab_1_private_rtb_association" {
  for_each       = aws_subnet.lab-1-private-subnet
  subnet_id      = each.value.id
  route_table_id = aws_route_table.lab_1_private_rtb.id
}

# Private Route Table association for database subnets
resource "aws_route_table_association" "lab_1_database_rtb_association" {
  for_each       = aws_subnet.lab-1-database-subnet
  subnet_id      = each.value.id
  route_table_id = aws_route_table.lab_1_private_rtb.id
}
...
resource "aws_security_group" "lab_1_ec2_sg" {
  name        = "${local.project_name_prefix}-${local.environment}-lab1-ec2-sg"
...
resource "aws_security_group" "lab_1_rds_sg" {
  name        = "${local.project_name_prefix}-${local.environment}-lab1-rds-sg"
...
resource "aws_security_group" "lab_1_lambda_sg" {
  name        = "${local.project_name_prefix}-${local.environment}-lab1-lambda-sg"
...
resource "aws_security_group" "lab_1_packer_sg" {
  name        = "${local.project_name_prefix}-${local.environment}-lab1-packer-sg"
...
resource "aws_vpc_security_group_ingress_rule" "allow_port_3306" {
  security_group_id            = aws_security_group.lab_1_rds_sg.id
...
resource "aws_vpc_security_group_ingress_rule" "allow_lambda_sg" {
  security_group_id            = aws_security_group.lab_1_rds_sg.id
  referenced_security_group_id = aws_security_group.lab_1_lambda_sg.id
  from_port                    = 3306
  ip_protocol                  = "tcp"
  to_port                      = 3306
}
...
resource "aws_vpc_security_group_egress_rule" "allow_all_traffic_from_lambda_sg" {
  security_group_id = aws_security_group.lab_1_lambda_sg.id
  cidr_ipv4         = "0.0.0.0/0"
  ip_protocol       = "-1" # semantically equivalent to all ports
}
...
resource "aws_vpc_security_group_egress_rule" "allow_all_traffic_from_packer_sg" {
  security_group_id = aws_security_group.lab_1_packer_sg.id
  cidr_ipv4         = "0.0.0.0/0"
  ip_protocol       = "-1" # semantically equivalent to all ports
}
...
#   owners = ["amazon"]
# }
# Deprecated above data block in favor of packer built AMI below
...
resource "aws_instance" "lab1_ec2_instance" {
  ...
  subnet_id              = aws_subnet.lab-1-private-subnet["lab_1_private_subnet_1"].id
  iam_instance_profile   = aws_iam_instance_profile.lab1_ec2_instance_profile.name
  vpc_security_group_ids = [aws_security_group.lab_1_ec2_sg.id]
  tags = {
...
resource "aws_db_subnet_group" "lab1_rds_subnet_group01" {
  name       = "${local.project_name_prefix}-${local.environment}-rds-subnet-group01"
  subnet_ids = [for i in aws_subnet.lab-1-database-subnet : i.id]
...
resource "aws_db_instance" "lab1-rds01" {
  ...
  db_subnet_group_name   = aws_db_subnet_group.lab1_rds_subnet_group01.name
  vpc_security_group_ids = [aws_security_group.lab_1_rds_sg.id]
...
resource "aws_ssm_parameter" "lab1_db_endpoint_param" {
...
resource "aws_ssm_parameter" "lab1_db_port_param" {
...
resource "aws_ssm_parameter" "lab1_db_name_param" {
...
# Explanation: Secrets Manager is lab1’s locked holster—credentials go here, not in code.
...
resource "aws_secretsmanager_secret_rotation" "rotation" {
  secret_id           = aws_secretsmanager_secret.armageddon_db_secret01.id
  rotation_lambda_arn = aws_lambda_function.lab1_lambda_secret_rotation_function.arn
...
resource "aws_lambda_function" "lab1_lambda_secret_rotation_function" {
  function_name    = "lab1_lambda_secret_rotation_function"
  ...
  vpc_config {
    ...
    security_group_ids = [aws_security_group.lab_1_lambda_sg.id]
  }
}
...
resource "aws_lambda_permission" "secretsmanager_invoke" {
  ...
  function_name = aws_lambda_function.lab1_lambda_secret_rotation_function.function_name
...
resource "aws_cloudwatch_log_group" "lab1_log_group01" {
